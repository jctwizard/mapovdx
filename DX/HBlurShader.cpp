...
HBlurShader::HBlurShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	InitShader(L"../shaders/hblur_vs.hlsl", L"../shaders/hblur_ps.hlsl");
}
...
void HBlurShader::InitShader(WCHAR* vsFilename, WCHAR* psFilename)
{
...
void HBlurShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, float textureWidth, float textureHeight)
{
	HRESULT result;
	...
	textureSizePtr->padding = XMFLOAT2(1.0f, 1.0f);
...
void HBlurShader::Render(ID3D11DeviceContext* deviceContext, int indexCount)
{
